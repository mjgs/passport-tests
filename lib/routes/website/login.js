const debug = require('debug')('app:lib:routes:website:login'); // eslint-disable-line no-unused-vars
...
router.get('/login', csrfProtection, function login(req, res) {
  const locals = {
...
router.post('/login', csrfProtection, passport.authenticate('local'), function processLogin(req, res, next) {
...
router.get('/users/:username', middleware.ensureAuthenticated(), function profile(req, res) {
